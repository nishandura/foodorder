...
<main class="ttr-wrapper">
	<div class="container-fluid">
		...
		if (isset($_POST['submit'])) {
			...
			if ($title != "" && $content != "" && $img != "" && $eventDate != "" && $venue != "") {

				$insert_query = "INSERT INTO events (business_id, title, content, img, eventDate, venue)
				 VALUES ('$business_id','$title', '$content', '$img', '$eventDate', '$venue')";
				$insert_result = mysqli_query($conn, $insert_query);
				echo $insert_result;
				if ($insert_result) {
		?>
					<meta http-equiv="refresh" content="0,url=manageevent.php?msg=aeventsuccess">
		<?php
				} else {
					echo "Event couldn't be added.";
				}
			} else {
						...
						<form class="edit-profile m-b30" form action="#" method="POST" enctype="multipart/form-data">
							<div class="row">
								...
								<div class="form-group col-12">
									...
									<div>
                                    <select id="inputProvience" class="form-control form-select1 " name="business_id"
                                        style="width:100%">
                                        <option disabled selected>Select business name</option>
                                        <?php
                                                $category="SELECT  * from businesslists";
                                                $c_result= mysqli_query($conn, $category);
                                                while($catagory_data=mysqli_fetch_array( $c_result)){
                                            ?>
                                        <option value="<?php echo $catagory_data['id']; ?> " selected>
                                            <?php echo $catagory_data['business_name']; ?></option>
                                        <?php
                                            }
                                            ?>
                                    </select>
								...
								<div class="form-group col-md-6">
									...
									<input type="date" name="eventDate" class="form-control" placeholder="" aria-describedby="helpId">
								</div>
